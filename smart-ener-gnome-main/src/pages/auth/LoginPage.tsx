...
export function LoginPage() {
  ...
  if (loading) {
    return <div>Loading...</div>; // Or use FullPageLoader
  }
  ...
  return (
    <AuthLayout
      ...
      footer={
        <>
          New to the platform?{" "}
          <Link to="/signup" className="text-primary hover:text-accent transition">
            Request access
          </Link>
        </>
      }
    >
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        >
          {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Access console"}
        </Button>
      </form>
    </AuthLayout>
  );
